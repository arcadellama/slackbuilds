...
update () {
	...
	DL_ARRAY64=($DOWNLOAD_x86_64)
	OLDMD564=($MD5SUM_x86_64)
	for i in "${!DL_ARRAY[@]}"; do
		NEWMD564=$(curl -sL ${DL_ARRAY64[$i]} | md5sum | cut -d ' ' -f 1)
		if [ $? -ne 0 ]; then
		    echo "Error downloading ${DL_ARRAY64[$i]}"
		    exit 1
		fi
		sed -i "s|${OLDMD564[$i]}|${NEWMD564}|g" $PRGNAM.info 
	done
    fi
